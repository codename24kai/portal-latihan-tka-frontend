...
export default function NotificationDropdown() {
  ...
  const colorTheme = userRole === 'admin' ? 'indigo' : userRole === 'guru' ? 'orange' : 'teal';

  const theme = {
    buttonActive: colorTheme === 'indigo' ? 'bg-indigo-600 shadow-indigo-200' : colorTheme === 'orange' ? 'bg-orange-600 shadow-orange-200' : 'bg-teal-600 shadow-teal-200',
    buttonHover: colorTheme === 'indigo' ? 'hover:border-indigo-200 dark:hover:border-indigo-900' : colorTheme === 'orange' ? 'hover:border-orange-200 dark:hover:border-orange-900' : 'hover:border-teal-200 dark:hover:border-teal-900',
    textHover: colorTheme === 'indigo' ? 'hover:text-indigo-600 dark:hover:text-indigo-400' : colorTheme === 'orange' ? 'hover:text-orange-600 dark:hover:text-orange-400' : 'hover:text-teal-600 dark:hover:text-teal-400',
    textGroupHover: colorTheme === 'indigo' ? 'group-hover:text-indigo-600 dark:group-hover:text-indigo-400' : colorTheme === 'orange' ? 'group-hover:text-orange-600 dark:group-hover:text-orange-400' : 'group-hover:text-teal-600 dark:group-hover:text-teal-400',
    borderLeft: colorTheme === 'indigo' ? 'bg-indigo-500' : colorTheme === 'orange' ? 'bg-orange-500' : 'bg-teal-500'
  };

  const handleSeeAll = () => {
    let target = '/notifikasi';
    if (userRole === 'guru') target = '/guru/notifikasi';
    if (userRole === 'admin') target = '/admin/notifikasi';
    navigate(target);
    setIsOpen(false);
  };
    ...
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className={`
          ...
          ${isOpen
            ? `${theme.buttonActive} text-white shadow-lg dark:shadow-none`
            : `bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-100 dark:border-slate-700 ${theme.buttonHover} shadow-sm hover:shadow-md`
          }
        `}
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className={`
            ...
            ${isOpen ? 'bg-white border-transparent' : 'bg-rose-500 border-white dark:border-slate-800'}
          `} />
        )}
      </button>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-3 w-80 bg-white dark:bg-slate-900 shadow-2xl rounded-2xl border border-slate-100 dark:border-slate-800 z-[60] overflow-hidden animate-in fade-in zoom-in duration-200 origin-top-right">
                ...
                {mockNotifs.map((notif) => (
                  <div
                    ...
                    className={`
                      ...
                      ${!notif.isRead ? 'bg-slate-500/5 dark:bg-slate-500/10' : ''}
                    `}
                  >
                    {!notif.isRead && (
                      <div className={`absolute left-0 top-0 bottom-0 w-1 ${theme.borderLeft}`} />
                    )}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 mb-0.5">
                        <p className={`text-xs font-black text-slate-800 dark:text-slate-200 truncate ${theme.textGroupHover} transition-colors`}>
                          {notif.title}
                        </p>
          ...
          <button
            ...
            className={`w-full py-3 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] ${theme.textHover} transition-colors border-t border-slate-50 dark:border-slate-800 bg-white dark:bg-slate-900`}
          >
            Lihat Semua Riwayat
          </button>
        </div>
