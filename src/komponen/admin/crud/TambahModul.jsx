...
// Sub Components
import QuizBuilder from '@/komponen/admin/ManajemenModul/PembuatKuis';
import FilePreviewUpload from '@/komponen/ui/UnggahPratinjauFile';

// Constants
import { SUBJECTS, SUBJECT_CATEGORIES } from '@/konstanta/mataPelajaran';
import { mockMateriData } from '@/data/mockMateri';
...
export default function AddModule() {
  ...
  useEffect(() => {
    if (isEdit) {
      const fetchModuleDetail = async () => {
        try {
          const res = await api.get(`/admin/modul/${id}`);
          const data = res.data;
          
          setFormData({
            title: data.title || '',
            category: data.category || SUBJECT_CATEGORIES.AKADEMIK,
          ...
          toast.error('Data modul tidak ditemukan');
  ...
  const handleSave = async (status) => {
    ...
    try {
      ...
      if (isEdit) {
        await api.put(`/admin/modul/${id}`, payload);
        toast.success('Materi berhasil diperbarui');
      } else {
        await api.post('/admin/modul', payload);
        toast.success(`Materi berhasil disimpan sebagai ${status === 'draft' ? 'Draft' : 'Rilisan'}`);
      }
      setTimeout(() => navigate('/admin/modul'), 1500);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="animate-fade-in space-y-8 pb-32 max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-black text-slate-800 dark:text-white uppercase tracking-tight italic">
            {isEdit ? 'Edit' : 'Tambah'} <span className="text-indigo-600">Materi Belajar</span>
          </h1>
            ...
            <div className="flex bg-slate-100 dark:bg-slate-900/50 p-1 rounded-xl border border-slate-200 dark:border-slate-700">
              {['video', 'pdf'].map(type => (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${formData.type === type
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-4 mb-4">
              <button
                ...
                className={`flex-1 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all border ${uploadMode === 'url' ? 'bg-indigo-600 text-white border-indigo-600 shadow-lg shadow-indigo-600/20' : 'bg-transparent text-slate-400 border-slate-100 dark:border-slate-700 hover:bg-slate-50'
                  }`}
              >
                Link URL
              </button>
              <button
                ...
                className={`flex-1 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all border ${uploadMode === 'file' ? 'bg-teal-600 text-white border-teal-600 shadow-lg shadow-teal-600/20' : 'bg-transparent text-slate-400 border-slate-100 dark:border-slate-700 hover:bg-slate-50'
                  }`}
              >
                Upload File
              </button>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] p-10 border border-slate-100 dark:border-slate-700 space-y-8 shadow-sm">
          ...
          {formData.hasQuiz && (
            <div className="animate-fade-in pt-4">
              <QuizBuilder quizData={quizData} setQuizData={setQuizData} openMathEditor={openMathEditor} />
            </div>
          )}
        </div>
      </div>
      ...
      <div className="mt-12 w-full">
        <div className="bg-slate-900/95 dark:bg-slate-800/95 backdrop-blur-xl border-t-4 border-indigo-500 rounded-[2.5rem] px-8 md:px-12 py-8 shadow-xl flex flex-col md:flex-row items-center justify-between gap-8 overflow-hidden relative">
          ...
          <div className="flex items-center gap-2 md:gap-4 w-full md:w-auto justify-center md:justify-end">
            <button onClick={() => navigate('/admin/modul')} className="px-6 py-4 text-slate-400 hover:text-white text-[10px] font-black uppercase tracking-widest transition-colors mr-2 md:mr-6">Batal</button>
            <div className="flex items-center gap-3">
              <button onClick={() => handleSave('draft')} className="px-8 py-4 bg-slate-800 dark:bg-slate-700 hover:bg-slate-700 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shadow-lg active:scale-95 border border-white/5">Draft</button>
      ...
      <VisualMathEditor isOpen={isMathOpen} onCancel={() => setIsMathOpen(false)} onInsert={handleInsertMath} />
    </div>
  );
}
