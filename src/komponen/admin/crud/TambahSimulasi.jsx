...
export default function AddTryout() {
  ...
  useEffect(() => {
    if (isEdit) {
      const fetchDetail = async () => {
        try {
          const res = await api.get(`/admin/sesi-latihan/${id}`);
          ...
          setFormData({
            ...
            selectionMethod: data.selectionMethod || 'auto',
            attachedSurveyId: 'none', // Dummy for now
            token: data.token || ''
          });
          ...
          if (data.soal_ids) {
            setSelectedQuestionIds(data.soal_ids);
          }
        } catch (err) {
  ...
  const handleGenerateToken = () => {
    ...
    const existingTokens = Object.values(storedTokens).map(t => String(t).toUpperCase());
    
    // Add default mock tokens to existing list to ensure uniqueness
    existingTokens.push('TKA202701', 'TKA202702', 'TKA2026');
    
    let nextNum = 1;
    let tokenValue = '';
    while (true) {
      const padNum = String(nextNum).padStart(2, '0');
      const candidate = `TKA2027${padNum}`;
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
          const res = await api.get('/admin/soal', { params: { per_page: 500 } });
          // Map data agar sesuai format yang dibutuhkan UI (text vs isi_soal)
          const mapped = res.data.data.map(q => ({
            id: q.id_soal,
            subject: q.mataPelajaran?.nama_mapel || q.mata_pelajaran?.nama_mapel || q.subject || 'Matematika',
            category: 'Akademik',
            difficulty: 'Sedang', // Mock difficulty for UI
            text: q.isi_soal,
          usedIn: false // Simplification
        }));
        
        // Filter based on confirmedConfig subject
        setAvailableQuestions(mapped.filter(q => q.subject === confirmedConfig.subject));
      } catch (err) {
        console.error('Gagal memuat bank soal', err);
      }
    };
    if (confirmedConfig.subject) {
      fetchQuestions();
    }
  ...
  const executeSave = async (status) => {
    try {
      ...
      if (isEdit) {
        await api.put(`/admin/sesi-latihan/${id}`, payload);
        toast.success(`Simulasi TKA berhasil diperbarui sebagai ${status === 'draft' ? 'Draft' : 'Rilisan'}`);
      } else {
        await api.post('/admin/sesi-latihan', payload);
        toast.success(`Simulasi TKA berhasil dibuat sebagai ${status === 'draft' ? 'Draft' : 'Rilisan'}`);
      }
      setIsConfirmOpen(false);
      setTimeout(() => navigate('/admin/sesi-latihan'), 1500);
    } catch (err) {
      console.error(err);
    ...
    <div className="animate-fade-in space-y-8 pb-32 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/admin/sesi-latihan')} className="w-12 h-12 rounded-2xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 flex items-center justify-center text-slate-400 hover:text-indigo-600 shadow-sm transition-all">
            <ArrowLeft size={20} />
          </button>
          <div>
      ...
      <div className="space-y-8">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] p-10 border border-slate-100 dark:border-slate-700 space-y-8 shadow-sm relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2 md:col-span-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Token Sesi (Kode Akses)</label>
              <div className="flex gap-4">
                <input
                  type="text"
                  value={formData.token}
                  onChange={(e) => setFormData({ ...formData, token: e.target.value.toUpperCase() })}
                  placeholder="Klik 'Generate Token' atau masukkan token kustom"
                  className="flex-1 px-5 py-4 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-700 rounded-2xl text-xs font-bold outline-none focus:ring-4 focus:ring-indigo-500/10 dark:text-white transition-all"
                />
                <button
                  type="button"
                  onClick={handleGenerateToken}
                  className="px-6 py-4 bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-900/30 dark:hover:bg-indigo-900/50 text-indigo-600 dark:text-indigo-400 rounded-2xl text-xs font-black uppercase tracking-widest transition-all active:scale-95 border border-indigo-100 dark:border-indigo-800/50 shrink-0"
                >
                  Generate Token
                </button>
              </div>
      ...
      <div className="w-full">
        <div className="bg-slate-900 rounded-[2.5rem] p-8 md:p-12 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-8 border-t-4 border-indigo-500 relative overflow-hidden">
          ...
          <div className="flex items-center gap-6 relative z-10 w-full md:w-auto">
            <button onClick={() => navigate('/admin/sesi-latihan')} className="text-slate-500 hover:text-white font-black text-xs uppercase tracking-widest px-6 transition-all">Batal</button>
