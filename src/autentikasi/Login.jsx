...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    // PELACAK 1
    console.log("1. Tombol diklik. Mencoba login dengan username:", username);

    try {
      const user = await login(username, password);

      // PELACAK 4
      console.log("4. Berhasil mendapat data dari Konteks:", user);
      ...
      if (user.role === 'admin') {
        ...
        // PELACAK 5
        console.log("5. Mengarahkan siswa ke halaman /beranda...");
        navigate('/beranda');
      }
    } catch (err) {
      // PELACAK ERROR
      console.error("ERROR TERTANGKAP DI LOGIN.JSX:", err);
