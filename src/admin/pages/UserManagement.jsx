...
export default function UserManagement() {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-20">
      ...
      {/* IMPORT PREVIEW SECTION - Refactored from Modal to Bottom Section */}
      {isImportModalOpen && (
        <div className="mt-12 animate-in slide-in-from-bottom-5 duration-500">
          <div className="w-full bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden flex flex-col">
            ...
            <div className="p-8">
              <div className="bg-slate-50/50 dark:bg-slate-900/30 rounded-[2rem] border border-slate-100 dark:border-slate-700 overflow-hidden">
                <DataTable
                  ...
                  rowsPerPage={50}
                  renderRow={(item, idx) => (
                    <tr key={item?.id} className="border-b border-slate-100/50 dark:border-slate-700/50 hover:bg-white dark:hover:bg-slate-800 transition-colors">
                      ...
                      <td className="py-4 px-6 text-center">
                        <Badge text={item?.role === 'admin' ? 'Admin' : 'Siswa'} variant={item?.role === 'admin' ? 'Info' : 'Success'} className="!text-[8px]" />
                      </td>
                      <td className="py-4 px-6 text-center">
            ...
            <div className="p-10 bg-slate-50/50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-700 flex flex-col sm:flex-row items-center justify-between gap-6 shrink-0">
              <div className="flex items-center gap-3">
                <ShieldCheck size={20} className="text-emerald-500" />
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-tight">Data akan diproses dan akun akan langsung aktif setelah konfirmasi.</p>
              </div>
              <div className="flex items-center gap-4 w-full sm:w-auto">
                <button
                  ...
                  className="flex-1 sm:flex-none px-8 py-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl text-[10px] font-black uppercase tracking-widest text-slate-500 transition-all hover:bg-slate-50"
                >
                  Batalkan
                </button>
                <button
                  onClick={confirmImport}
                  className="flex-1 sm:flex-none px-10 py-4 bg-emerald-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-emerald-600/20 hover:bg-emerald-700 transition-all active:scale-95"
                >
