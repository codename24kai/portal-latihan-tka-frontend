...
export default function QuestionBank() {
  ...
  const filteredData = useMemo(() => {
    ...
    let result = questions.filter((q) => {
      const matchCategory = activeTab === 'Riwayat Penggunaan'
        ? (q.usedIn !== null && q.usedIn !== undefined && q.usedIn !== '')
    ...
    <div className="animate-fade-in space-y-6 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 px-6 py-3 bg-rose-50 dark:bg-rose-900/20 border border-rose-200 dark:border-rose-800 rounded-2xl text-[10px] font-black uppercase tracking-widest text-rose-600 dark:text-rose-400 hover:bg-rose-100 dark:hover:bg-rose-900/40 transition-all active:scale-95"
          >
            <FileText size={16} /> Pdf Import
          </button>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
        <div className="flex items-center gap-8 px-10 py-5 border-b border-slate-50 dark:border-slate-700">
          {['Akademik', 'Survei', 'Riwayat Penggunaan'].map((tab) => (
            <button
              key={tab}
          ...
          headers={
            activeTab === 'Riwayat Penggunaan'
              ? [
                ...
                { label: 'Tempat Penggunaan', align: 'center' },
                { label: 'Frekuensi Penggunaan', align: 'center' },
          ...
          renderRow={(q) => {
            ...
            return (
              <tr key={q.id} className="group hover:bg-slate-50/50 dark:hover:bg-slate-900/30 transition-all cursor-pointer" onClick={() => setPreviewQuestion(q)}>
                ...
                {activeTab === 'Riwayat Penggunaan' ? (
                  <>
                    <td className="py-6 px-4 text-center text-xs font-bold text-slate-600 dark:text-slate-350">{places.join(', ')}</td>
                    <td className="py-6 px-4 text-center">
                      <Badge
                        text={`${useCount}x Digunakan`}
                        variant={useCount > 1 ? 'Danger' : 'Success'}
                      />
                    </td>
                  </>
                ) : (
                ...
                <td className="py-6 px-8 text-right" onClick={(e) => e.stopPropagation()}>
                  <div className="flex justify-end items-center gap-2">
                    ...
                    <button onClick={() => navigate(`/admin/bank-soal/edit/${q.id}`)} className="px-3 py-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded-xl transition-all font-black text-[10px] uppercase flex items-center gap-1.5"><Edit2 size={14} /> Edit</button>
