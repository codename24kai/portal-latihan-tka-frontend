...
export default function AddEditSurvey() {
  ...
  const handleSave = (status) => {
    ...
    const payload = {
      ...formData,
      status,
      id: isEdit ? formData.id : `survey-${Date.now()}`
    };

    if (isEdit) {
      const idx = mockSurveyDefinitions.findIndex(s => s.id === formData.id);
      if (idx !== -1) {
        mockSurveyDefinitions[idx] = payload;
      }
      toast.success('Survei berhasil diperbarui!');
    } else {
      mockSurveyDefinitions.push(payload);
      toast.success('Survei berhasil disimpan!');
    }

    setTimeout(() => navigate('/admin/survei'), 1500);
  };
