  ...
  return (
    <div className="space-y-8 pt-4 animate-in slide-in-from-top-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-full px-5 py-3.5 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-700 rounded-2xl text-[11px] font-bold outline-none dark:text-white focus:ring-4 focus:ring-orange-500/10"
            />
          </div>
        </div>
      )}
      ...
      <div className="space-y-6">
              ...
              {q.options.map((opt, oIndex) => (
                <div key={oIndex} className="relative">
                  <div className="flex flex-col gap-2">
                    <div className="relative">
                      <div className={`absolute left-3 top-1/2 -translate-y-1/2 w-6 h-6 rounded-lg flex items-center justify-center text-[9px] font-black ${q.answer === oIndex ? 'bg-teal-500 text-white' : 'bg-slate-200 text-slate-500'}`}>
                  ...
                  {(typeof opt === 'string' ? opt : opt.text).includes('$') && (
                    <div className="absolute left-12 -bottom-10 z-10 px-4 py-2 bg-white dark:bg-slate-800 border border-teal-100 dark:border-teal-800 rounded-xl shadow-xl animate-in slide-in-from-top-1">
        ...
        <button
          onClick={addQuestion}
          className="w-full py-5 bg-orange-600 hover:bg-orange-700 text-white rounded-[1.5rem] flex items-center justify-center gap-3 text-[10px] font-black uppercase tracking-widest transition-all shadow-xl shadow-orange-600/20 active:scale-95 group"
        >
...
export default GuruQuizBuilder;
