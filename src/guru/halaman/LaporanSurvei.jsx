...
export default function SurveyReports() {
  ...
  useEffect(() => {
    ...
    api.get(`/guru/survei/${selectedSurveyId}/rekap`)
  ...
  const handleDownload = async () => {
    try {
      ...
      const response = await api.get(`/guru/survei/${selectedSurveyId}/export`, { responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.download = 'laporan-survei-guru.csv';
      link.click();
      ...
      {selectedResponseDetail && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-2xl bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-2xl border border-slate-100 dark:border-slate-700 overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            {/* HEADER SECTION - UPDATED WITH DYNAMIC TITLE */}
            <div className="px-8 py-6 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/50">
              <div className="flex items-center gap-4">
                {/* Avatar / Icon Placeholder */}
                <div className="w-12 h-12 rounded-full bg-indigo-100 dark:bg-indigo-900/50 flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-black text-sm">
                  {selectedResponseDetail?.name?.charAt(0)?.toUpperCase()}
                </div>
                <div>
                  {/* Judul Utama */}
                  <h3 className="text-base font-black text-slate-800 dark:text-white uppercase tracking-tight">
                    Respon: {selectedResponseDetail.name}
                  </h3>
                  {/* Subtitle */}
                  <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-0.5">
                    {selectedSurvey?.title} • Kelas {selectedResponseDetail.class}
                  </p>
                </div>
              </div>
              ...
              <button
                ...
                className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-slate-400 hover:text-rose-500 transition-all font-black text-xs"
              >
                ✕
              </button>
            </div>
                  ...
                  return (
                    <div key={q.id} className="space-y-3">
                      <p className="text-xs font-black text-slate-800 dark:text-white leading-tight">
                        <span className="text-indigo-600 mr-1">Q{idx + 1}.</span> {q.text}
                      </p>
